...
const fetchGReviews = async (chainId, address, id) => {
  ...
  const review = await fetch(`https://api.thegraph.com/subgraphs/name/ameer-clara/honft-test`, {
    method: 'POST',
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  ...
  setReviewFormStatus('Review submitted');

  // if (request.message === 'reviews_loaded') {
  //   reviewsLoaded = true;
  // }
});
...
let getAddressAndId = (link) => {
  ...
  if (address !== null && id != null) {
    ...
    console.log('Processing : ', address + ', id: ', cleanId + ', chain: ', chainId, ', key: ', key);
...
let sendPayloadToExtension = (nftPayload, isSubmission) => {
  ...
  chrome.runtime.sendMessage({ open: true }, async (response) => {
    ...
    chrome.runtime.sendMessage(nftPayload);
